...
#: ``probesize=32``, ``analyzeduration=0``) are not needed here and are set only
#: for a caller that opens a container: the frame protocol delivers whole access
#: units with an explicit length, so we feed a bare CodecContext and there is no
#: demuxer to probe, buffer or guess with in the first place.
