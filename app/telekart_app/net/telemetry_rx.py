"""Telemetry receive thread: decode, statistics, publish, record."""
...
class TelemetrySample:
    ...
    wall_t: float  # time.time, for the recorder
...
class TelemetryRxThread(threading.Thread):
    ...
    def set_session(self, session_id: int, key: bytes) -> None:
    ...
    def run(self) -> None:
        ...
        while not self._shutdown.is_set():
            ...
            try:
                packet = TelemetryPacket.unpack(view[:nbytes], key)
            ...
            sample = TelemetrySample(
                packet=packet, recv_t=recv_t, wall_t=time.time(), rtt=rtt
            )
            self._box.put(sample)

            recorder = self._recorder
            if recorder is not None:
                try:
                    recorder.record_telemetry(sample)
                except Exception as exc:  # recording is never worth the link
                    _throttle.log(
                        _log, logging.ERROR, "record", "telemetry recording failed: %s", exc
                    )
    ...
    def _measure_rtt(self, packet: TelemetryPacket, recv_t: float) -> float:
        ...
        if rtt <= 0.0 or rtt > _MAX_PLAUSIBLE_RTT:
            return 0.0
        with self._lock:
            self._rtt_ring[self._rtt_i] = rtt
            self._rtt_i = (self._rtt_i + 1) % len(self._rtt_ring)
            if self._rtt_n < len(self._rtt_ring):
                self._rtt_n += 1
            self._rtt_last = rtt
        return rtt
    ...
    def stats(self) -> TelemetryStats:
        ...
        with self._lock:
            ...
            rtt_min = 0.0
            if n:
                # Sorting 256 floats is ~10 us, and stats() runs at 20 Hz on
                # the supervisor thread -- not a path worth optimising.
                window = sorted(self._rtt_ring[:n])
                idx = int(0.95 * n) - 1
                rtt_p95 = window[idx if idx >= 0 else 0]
                rtt_min = window[0]
