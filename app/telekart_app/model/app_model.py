...
from ..race.director import EMPTY_RACE, RaceDirector, RaceSnapshot
...
class SnapshotSources(Protocol):
    ...
    car, no simulator. That is the property that makes screens testable.
    """
...
class AppModel(QObject):
    ...
    * **One drain per frame.** All five boxes are read in one pass, so every
      ...
      per packet instead would let the speedometer and the RPM bar come from
      different packets, which reads on screen as a drivetrain fault that is
      not there.
    ...
    raceChanged = Signal(RaceSnapshot)
    ...
    #: An ``input.mapping.Action`` (as an int) that the model does not handle
    #: itself -- MARK_LAP, TOGGLE_HUD, CYCLE_CAMERA, TOGGLE_RECORD. The window
    #: owns those because they are window-level behaviours.
    ...
    def __init__(
        ...
        *,
        ...
        # is the point -- INTERFACES.md 9 says widgets talk to this and to
        # nothing else, so the network has to live behind it either way.
        if runtime is None and settings is not None:
            from .runtime import AppRuntime

            runtime = AppRuntime(settings, profiles=profiles)
        ...
        if runtime is not None:
            ...
            race = race if race is not None else runtime.race
        ...
        self._race_snapshot = EMPTY_RACE
    ...
    def set_race(self, race: RaceDirector | None) -> None:
        self._race = race
    ...
    @property
    def race(self) -> RaceDirector | None:
        return self._race
    ...
    @property
    def race_snapshot(self) -> RaceSnapshot:
        return self._race_snapshot
    ...
    def _drain_input_events(self) -> None:
        ...
        for action in runtime.take_input_events():
            ...
            if handler is not None:
                ...
                self.actionTriggered.emit(int(action))
    ...
    def _drain_telemetry(self, now: float) -> None:
        ...
        if snapshot.state is not previous.state:
            ...
            race = self._race
            if race is not None:
                race.note_vehicle_state(snapshot.state)
    ...
    def _drain_race(self, now: float) -> None:
        race = self._race
        if race is None:
            return
        snapshot = race.tick(now)
        if not differs(self._race_snapshot, snapshot):
            return
        self._race_snapshot = snapshot
        self.raceChanged.emit(snapshot)
    ...
    def request_params(self, group: str = "") -> None:
        ...
        if controller is None:
            _log.info("no controller: ignoring request_params")
            return
        controller.request_params(group)

    def set_params(self, values: Mapping[str, Any]) -> int:
        """Push parameters. The car's PARAMS reply is what the UI must render.

        Returns the request id, or -1 with no controller. Never optimistically
        updates the local parameter view: a UI showing a value the car refused
        is how you end up tuning the wrong vehicle.
        """
        controller = self._controller
        if controller is None:
            _log.info("no controller: ignoring set_params")
            return -1
        return controller.set_params(values)
    ...
    def connect_to(self, host: str = "", *, shared_key: str = "", driver: str = "") -> None:
        runtime = self._runtime
        if runtime is None:
            _log.info("no runtime: ignoring connect_to(%r)", host)
            return
        try:
            runtime.link.connect(host, shared_key=shared_key, driver=driver)
        except ValueError as exc:
            # A missing passphrase is an operator error, not a crash. The link
            # state carries the reason to the connect screen.
            _log.warning("cannot connect: %s", exc)
    ...
    def calibrate(self, routine: str = "drive", *, on_ground: bool = False) -> None:
        runtime = self._runtime
        if runtime is not None:
            runtime.link.calibrate(routine, on_ground=on_ground)
    ...
    def set_video_enabled(self, enabled: bool) -> None:
        runtime = self._runtime
        if runtime is not None:
            runtime.link.set_video_enabled(enabled)

    # -- input --------------------------------------------------------------

    def input_profile(self) -> Any:
        """The profile of the live device, or ``None`` when there is no runtime."""
        runtime = self._runtime
        return None if runtime is None else runtime.profile

    def set_input_profile(self, profile: Any) -> None:
        runtime = self._runtime
        if runtime is not None and profile is not None:
            runtime.set_profile(profile)

    @property
    def device_snapshot(self) -> Any:
        runtime = self._runtime
        return None if runtime is None else runtime.device_snapshot
...
#: Edge actions the model can service on its own. Everything else is a window
#: concern and goes out on ``actionTriggered``.
...
def _vehicle_from_packet(packet: TelemetryPacket, *, stale: bool) -> VehicleSnapshot:
    ...
    return VehicleSnapshot(
        ...
        flags=packet.flags,
        rpm_l=float(packet.rpm_l),
        rpm_r=float(packet.rpm_r),
        rpm_target_l=float(packet.rpm_target_l),
        rpm_target_r=float(packet.rpm_target_r),
        duty_l=packet.duty_l_f,
        duty_r=packet.duty_r_f,
        servo_us=packet.servo_us,
        steer_angle=_radians(packet.steer_angle_deg),
        speed=packet.speed_mps,
        v_max=packet.v_max_mps,
        pose=packet.pose_m,
        distance=packet.distance_m,
        slip=packet.slip,
