...
#: How each action behaves. HORN is held because that is what a horn is. REVERSE
#: is a toggle because holding a paddle through a three-point turn is miserable.
#: ESTOP is held *and* latched by the state machine on the car -- the app just
#: keeps asserting the bit for as long as the button is down, and the car
#: decides when it is allowed to come back out.
ACTION_MODES: dict[Action, ActionMode] = {
    ...
    Action.PIT_LIMITER: ActionMode.TOGGLE,
    Action.HEADLIGHTS: ActionMode.TOGGLE,
    Action.HORN: ActionMode.HOLD,
    Action.MARK_LAP: ActionMode.EDGE,
    Action.TOGGLE_HUD: ActionMode.TOGGLE,
    Action.CYCLE_CAMERA: ActionMode.EDGE,
    Action.TOGGLE_RECORD: ActionMode.TOGGLE,
}

#: Actions that map straight onto a control-packet flag. The rest are either
#: session-channel messages (ARM/DISARM/CLEAR_FAULTS) or purely local to the app.
ACTION_FLAGS: dict[Action, ControlFlags] = {
    ...
    Action.PIT_LIMITER: ControlFlags.PIT_LIMITER,
    Action.HEADLIGHTS: ControlFlags.HEADLIGHTS,
    Action.HORN: ControlFlags.HORN,
}

#: Actions the session client owns, in the order the wizard offers them.
...
ACTION_LABELS: dict[Action, str] = {
    ...
    Action.PIT_LIMITER: "Pit limiter",
    Action.HEADLIGHTS: "Headlights",
    Action.HORN: "Horn",
    Action.MARK_LAP: "Mark lap",
    Action.TOGGLE_HUD: "Show HUD",
    Action.CYCLE_CAMERA: "Next camera view",
    Action.TOGGLE_RECORD: "Record session",
}
...
class InputRef:
    ...
    def to_dict(self) -> dict[str, Any]:
        out: dict[str, Any] = {"kind": self.kind.value, "index": self.index}
        if self.kind is RefKind.HAT:
            out["hat_axis"] = self.hat_axis
            out["sign"] = self.sign
        return out
    ...
    def from_dict(cls, data: object) -> "InputRef":
        ...
        if not isinstance(data, dict):
            raise MappingError(f"input ref must be an object, got {type(data).__name__}")
        raw_kind = data.get("kind", "none")
        try:
            kind = RefKind(raw_kind)
        except ValueError as exc:
            raise MappingError(f"unknown input kind {raw_kind!r}") from exc
        return cls(
            kind=kind,
            index=_as_int(data.get("index", 0), "index"),
            hat_axis=_as_int(data.get("hat_axis", 0), "hat_axis"),
            sign=_as_int(data.get("sign", 1), "sign"),
        )
...
class ControlBinding:
    ...
    def to_dict(self) -> dict[str, Any]:
        out: dict[str, Any] = {"positive": self.positive.to_dict()}
        if self.negative.is_bound:
            out["negative"] = self.negative.to_dict()
        return out

    @classmethod
    def from_dict(cls, data: object) -> "ControlBinding":
        if not isinstance(data, dict):
            raise MappingError(
                f"control binding must be an object, got {type(data).__name__}"
            )
        return cls(
            positive=InputRef.from_dict(data.get("positive")),
            negative=InputRef.from_dict(data.get("negative")),
        )
...
class ActionBinding:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"action": self.action.name.lower(), "ref": self.ref.to_dict()}

    @classmethod
    def from_dict(cls, data: object) -> "ActionBinding":
        if not isinstance(data, dict):
            raise MappingError(
                f"action binding must be an object, got {type(data).__name__}"
            )
        raw = data.get("action", "")
        try:
            action = Action[str(raw).upper()]
        except KeyError as exc:
            raise MappingError(f"unknown action {raw!r}") from exc
        return cls(action=action, ref=InputRef.from_dict(data.get("ref")))
...
class InputMap:
    ...
    @classmethod
    def from_dict(cls, data: object) -> "InputMap":
        if not isinstance(data, dict):
            raise MappingError(f"input map must be an object, got {type(data).__name__}")
        raw_actions = data.get("actions", [])
        if not isinstance(raw_actions, (list, tuple)):
            raise MappingError("input map actions must be a list")
        return cls(
            steer=ControlBinding.from_dict(data.get("steer", {})),
            throttle=ControlBinding.from_dict(data.get("throttle", {})),
            brake=ControlBinding.from_dict(data.get("brake", {})),
            actions=tuple(ActionBinding.from_dict(a) for a in raw_actions),
        )
...
def describe_map(mapping: InputMap) -> Mapping[str, str]:
    """Flat description for a settings screen. No axis indices leak into the
    normal UI, but the wizard's summary page wants them."""
    out: dict[str, str] = {
        "Steering": mapping.steer.describe(),
        "Throttle": mapping.throttle.describe(),
        "Brake": mapping.brake.describe(),
    }
    for entry in mapping.actions:
        out[ACTION_LABELS[entry.action]] = entry.ref.describe()
    return out
