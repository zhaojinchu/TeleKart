...
class ControlTxThread(threading.Thread):
    ...
    def clear_target(self) -> None:
        """Stop transmitting. The session id is dropped, so nothing can go out
        naming a session the car has forgotten."""
        with self._tx:
    ...
    # There is no set_arm_intent. ControlFlags.ARM_INTENT exists on the wire but
    # no firmware code path reads it -- arming is decided entirely over the TCP
    # session -- so setting it was a bit the app spent 100 times a second to say
    # something nobody was listening for.
    ...
    def _transmit(
        ...
        with self._tx:
            ...
            if sock is None or not self._session_id:
    ...
    def stats(self) -> ControlTxStats:
        with self._tx:
            transmitting = self._sock is not None and bool(self._session_id)
