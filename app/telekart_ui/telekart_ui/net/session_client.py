"""TCP/JSON session channel: handshake, token derivation, commands.
...
**One connection attempt, no reconnect.** The thread connects, runs, and exits
when the session ends, for whatever reason. That follows directly from the
presence rule above: because the car E-stops when this socket drops, a silent
reconnect would put the app back on a car that is latched and refusing to arm,
with nothing on screen explaining why. The driver reconnects deliberately and
clears the E-stop deliberately. (The *video* stream does reconnect on its own --
see ``video/receiver.py`` -- because losing the picture is a degradation, not a
safety event.)
"""
...
class SessionClient(threading.Thread):
    ...
    # There is deliberately no get_params/set_params/calibrate. Parameters are
    # edited in pi/config/config.local.yaml and calibration is run on the car by
    # pi/scripts/calibrate_drive.py -- which is where the measured values always
    # came from. A station that can push a wrong number to a moving car buys
    # nothing that a text editor and a disarmed car do not.
    ...
    def run(self) -> None:
        """Connect once, pump until the session ends, exit.

        Every exit path emits exactly one terminal event, so the link manager
        never has to guess whether this thread is coming back. It is not.
        """
        self._emit(SessionEvent(SessionEventKind.CONNECTING, detail=self._address))
        sock = None
        # One LineReader across handshake and steady state: the nonce and the
        # hello_ack can arrive in the same TCP segment as the first STATE
        # message, and a fresh reader would drop the tail of it.
        reader = LineReader()
        try:
            sock = self._connect()
            ...
            self._fatal = True
            self._emit(SessionEvent(SessionEventKind.FAILED, detail=str(exc), code=exc.code))
            _close(sock)
            return
            ...
            with self._state_lock:
                self._last_error = str(exc)
            # A refused connect and a lost session are the same outcome for the
            # driver -- there is no car to drive and the next move is theirs --
            # so both surface as FAILED rather than one of them looking transient.
            self._emit(SessionEvent(SessionEventKind.FAILED, detail=str(exc)))
            return
    ...
    def _handshake(self, sock: socket.socket, reader: LineReader) -> SessionInfo:
        """Send hello, read hello_ack. One round trip, no challenge.

        Earlier revisions began with the car sending an unsolicited nonce which
        the app answered with an HMAC. That is gone: this build has no shared
        key, so the app speaks first and the car accepts anyone who can reach
        the port.
        """
        ...
        if not isinstance(session_id, int):
            raise SessionError("hello_ack is missing session_id")
    ...
    def _dispatch(self, message: Message) -> None:
        ...
        if message.type is MsgType.ERROR:
...
def _fatal_from_error(message: Message) -> Exception:
    ...
    if code == ErrorCode.PROTOCOL_VERSION.value:
