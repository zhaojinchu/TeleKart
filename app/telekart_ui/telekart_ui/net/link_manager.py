...
from telekart_protocol import MsgType, VehicleState, VideoCodec
...
class LinkManager:
    ...
    telemetry stale         STALE               allowed, but nothing on the
                                                HUD can be trusted
    session dropped         FAILED              no -- the target is dropped, so
                                                the TX thread stops sending
    version refused         FAILED              no
    ======================  ==================  ==========================
    ...
    becomes a guess. Session loss is decisive and terminal -- the firmware
    already treats a dropped TCP connection as an E-stop condition, so the app
    stops transmitting and waits for the driver rather than reconnecting into a
    car that is latched and will refuse to arm.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Chosen at launch rather than read back from the car. The car's own
        # video_codec parameter is read once by telekart-video at startup and
        # never renegotiated, so asking for it over the session channel only
        # ever confirmed what a flag already knows.
        self._codec = codec
    ...
    def connect(self, host: str = "", *, driver: str = "") -> None:
        ...
        with self._lock:
            ...
            self._set_state(LinkState.RESOLVING, self._host or discovery.DEFAULT_HOSTNAME)
    ...
    def _connect_worker(self) -> None:
        ...
        target = discovery.resolve(
            self._host or discovery.DEFAULT_HOSTNAME, timeout=link.discovery_timeout
        )
        if target is None:
            self._set_state(
                LinkState.FAILED,
        ...
        if not self._host:
            self._host = target.host or target.address
        ...
        if old is not None:
            old.close()
        self._set_state(LinkState.CONNECTING, target.address)
        session.start()

    def disconnect(self) -> None:
        """Tear the link down cleanly.
    ...
    def arm(self) -> None:
        ...
        The car checks its preconditions in a fixed order -- E-stop clear, no
        critical fault, valid session, throttle held neutral -- and names the
        one that failed. That string is what the HUD shows; the app does not
        second-guess it.
        """
    ...
    def _on_connected(self, info: SessionInfo) -> None:
        ...
        self._telemetry.set_session(info.session_id)
        try:
            self._control.set_target(
                info.address, info.session_id, port=info.control_port
            )
        except OSError as exc:
    ...
    def _on_disconnected(self, detail: str) -> None:
        ...
        if self._state is not LinkState.FAILED and self._state is not LinkState.OFFLINE:
            # Terminal, not transient. The car E-stopped the moment this socket
            # dropped, so the next step is the driver's: reconnect, then clear
            # the E-stop. Saying RECONNECTING here would promise a recovery that
            # is not coming and would hide the latch behind an optimistic label.
            self._set_state(LinkState.FAILED, detail or "session lost")

    def _teardown_channels(self) -> None:
        # Dropping the target is what stops the TX thread, and it matters more
        # now than it used to: with no per-session key, a stale sender is no
        # longer rejected by the car -- it is only *not sending*. This app must
        # therefore be scrupulous about going quiet the moment its session ends.
        # The firmware treats the lost TCP connection as an E-stop condition
    ...
    def _start_video(self, info: SessionInfo) -> None:
        ...
        if video is None:
            try:
                ...
                # and optional, and a headless test drives control and
                # telemetry with neither installed.
            ...
            video = VideoRxThread(self.video_box, codec=self._codec, target_size=size)
            self._video = video
            video.start()
    ...
    def _evaluate(self) -> None:
        ...
        self.link_box.put(
            LinkSnapshot(
                ...
                video_decode_ms=video_stats.decode_ms if video_stats else 0.0,
                video_ok=video_ok,
                session_ok=session_ok,
    ...
    def _classify(
        ...
        *,
        ...
        current = self._state
        if current in (LinkState.OFFLINE, LinkState.FAILED, LinkState.RESOLVING):
            return None, ""
        if not session_ok:
            if current is LinkState.CONNECTING:
                return None, ""
            return LinkState.FAILED, "session down"
        if session_age > SESSION_STALE_S:
            return LinkState.FAILED, "session silent"
