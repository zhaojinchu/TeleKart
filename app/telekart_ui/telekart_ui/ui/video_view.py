...
it drops a :class:`~telekart_ui.video.frame.FrameBundle` into a depth-1 box and
...
second, but a steering arc driven by 50 Hz telemetry and smoothed needs 60 Hz to
look like motion rather than stepping. Letting the decoder drive repaints would
invert that -- the HUD would stutter at camera rate and the GUI thread would be
woken by a thread it has no business being coupled to.

**The bundle is held for the whole paint.** ``FrameBundle`` owns an
``av.VideoFrame`` and a ``QImage`` that views its buffer without copying. Storing
the image and dropping the bundle is a use-after-free that renders correctly most
of the time. So the view keeps its own reference until the next one replaces it,
and takes from the provider exactly once per tick rather than peeking twice.
...
from .theme import C, SIZE_BODY, SIZE_MICRO, SIZE_TITLE, ui_font, with_alpha
...
class VideoView(QWidget):
    ...
    makes Qt repaint the picture beneath a HUD zone when the zone updates; two
    siblings in the same backing store do not blend, and the overlay would
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._font_placeholder = ui_font(SIZE_TITLE, QFont.Weight.Medium)
        self._font_detail = ui_font(SIZE_BODY)
        self._font_badge = ui_font(SIZE_MICRO, QFont.Weight.DemiBold)
    ...
    def tick(self) -> bool:
        ...
        The HUD does not need calling: its zones animate themselves and Qt
        repaints them over whatever this widget just drew.
        """
    ...
    def _place_hud(self) -> None:
        ...
        The overlay's margin rule is written against the picture edge -- cameras
        vignette, and an instrument hard against the frame edge sits in the
        darkest, least stable part of the image. On a letterboxed stream the
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        if self._stale:
            ...
            self._paint_badge(painter, target, "recovering", C.warn)
    ...
    def _paint_placeholder(self, painter: QPainter) -> None:
        ...
        if self._placeholder_detail:
            ...
            painter.setPen(C.text_tertiary)
            painter.drawText(
    ...
    def _paint_stale(self, painter: QPainter, target: QRectF) -> None:
        painter.fillRect(target, with_alpha(C.bg_base, 0.55))
        self._paint_badge(painter, target, "last frame", C.bad)
