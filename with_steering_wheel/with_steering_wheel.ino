/*
  ESP32 WiFi RC Car Controller - V6
  Changes:
  - Throttle shaping (less sensitive): deadband + expo curve + optional minimum-start bump
  - Reverse enabled works with shaping (gas - brake, then shaped)
  - Steering center adjustable live (microseconds slider) + trim adjustable live
  - “Recenter” button: bakes current trim into center and resets trim to 0
  - Pedals supported as ANALOG BUTTONS (B6/B7) or AXES
  - Controller selector so you can pick the right device when multiple are connected
  - Sliders follow wheel/pedals so you see live values
...
// Base center you can change live from the webpage (microseconds)
int steerCenterUs = 2000; // starting guess; adjust on the web UI

// Trim: -100..100 -> microseconds shift (fine adjust)
static const int STEER_TRIM_US_PER_UNIT = 3;  // must match JS constant
...
// ================== DEFAULT UI VALUES ==================
int steeringTrim = 0;      // live-adjust on web UI
...
volatile int targetThrottlePct = 0; // -100..100
volatile int targetSteerPct    = 0; // -100..100
...
void applyOutputs(int throttlePct, int steerPct) {
  ...
  // -------- Steering: EVEN around the current center --------
  ...
  int travelUs   = (leftAvail < rightAvail) ? leftAvail : rightAvail; // force symmetry

  int steerUs = centerUs + (travelUs * steerPct) / 100;
...
<div class="card">
  ...
  <div class="small">Use <b>Center</b> for big correction, <b>Trim</b> for small correction. After a crash, Center often needs a tweak.</div>
...
<div class="card">
  <b>Pedals setup</b>
  <div class="small">
    You said pedals are <b>B7 (throttle)</b> and <b>B6 (brake)</b>. Default is set for that.
  </div>
...
<div class="card">
  <b>Throttle sensitivity (make it less twitchy)</b>
  <div class="small">
    If you need a “push” before the car moves, set a <b>Min Start</b>. If it's twitchy, increase <b>Expo</b>.
  </div>
...
<div class="card">
  ...
  <div class="small">W/S for forward/reverse, A/D to steer (keyboard is mostly for backup)</div>
</div>
...
function syncPedalVars() {
  ...
  pedalStatusEl.innerText =
    ...
    " | shaping: deadband=" + TH_DEADBAND + "% expo=" + TH_EXPO.toFixed(1) + " minStart=" + TH_MIN_START + "%";
}
...
void setup() {
  ...
  // start safe
  steerServo.writeMicroseconds(clampInt(steerCenterUs, STEER_MIN_US, STEER_MAX_US));
